...
export class Family {
    constructor() {
        ...
        this.apiBase = 'http://localhost:5000/api/family';
        this.members = [];
    ...
    async downloadPhoto(photoData, title) {
        try {
            ...
            this.showToast('⚠️ Error downloading photo', 'error');
        }
    }
    ...
    async handlePhotoSelection(files) {
        ...
        if (files.length > this.maxPhotosPerUpload) {
            this.showToast(`⚠️ Maximum ${this.maxPhotosPerUpload} photos allowed`, 'error');
            return;
        }
        ...
        if (validFiles.length === 0) {
            this.showToast('⚠️ No valid files to upload', 'error');
            return;
        }
    ...
    async saveNewMember() {
        ...
        if (!name) {
            ...
            this.showToast('⚠️ Please enter a member name', 'error');
            setTimeout(() => {
    ...
    async savePhotos() {
        ...
        if (!memberId) {
            this.showToast('⚠️ Please select a family member', 'error');
        ...
        if (!title) {
            this.showToast('⚠️ Please enter a photo title', 'error');
        ...
        if (this.uploadedPhotos.length === 0) {
            this.showToast('⚠️ Please select photos to upload', 'error');
            return;
        }
        ...
        body.innerHTML = `
            <div class="gallery-container">
                ...
                <div class="gallery-stats">
                    ...
                    <span>❤️ ${member.gallery ? member.gallery.filter(p => p.story).length : 0} Stories</span>
                    ${member.gallery && member.gallery.length > 0 ? 
    ...
    setupEventListeners() {
        ...
        document.querySelectorAll('.family-heart').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                this.showToast(`❤️ Sending love to ${member ? member.name : 'your family member'}!`, 'success');
            });
        });
